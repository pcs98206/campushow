...
import { protectMiddleware, publicMiddleware, uploadFilesMulter, uploadAvatarsMulter, uploadThumbnailMulter } from "../middlewares";

const globalRouter = express.Router();

globalRouter.get("/", handleHome);
globalRouter.route("/sell").all(protectMiddleware).get(getSell).post(uploadFilesMulter.single('uploadFile'), postSell);
globalRouter.route("/sell/:id([0-9a-f]{24})").all(protectMiddleware).get(getEdit).post(uploadThumbnailMulter.single('uploadThumbnail'), postEdit);
